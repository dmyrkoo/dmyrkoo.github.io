...
function ArticleCard({
  ...
  const handleToggleLike = async () => {
    if (!isAuthenticated) {
      onOpenAuthModal();
      setLikeError('Увійдіть, щоб вподобати');
      return;
    }

    if (isLiked || isLikePending) {
      setLikeError('Ви вже вподобали цю статтю');
      return;
    }
    ...
    if (response.status === 201) {
      onLikeSuccess(article.id);
      return;
    }

    if (response.status === 400) {
      const payload = await response.json();
      setLikeError(payload.message || 'Ви вже вподобали цю статтю');
      return;
    }

    setLikeError('Не вдалося додати вподобання. Спробуйте ще раз.');
  };
  ...
  return (
    <article className={`card ${isLiked ? 'liked' : ''}`}>
      ...
      <div className="card-body">
        ...
        <div className="article-meta" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '15px 0' }}>
          ...
          <button
            ...
            disabled={isLikePending || isLiked}
          >
            {isLiked ? 'Подобається (вподобано)' : 'Подобається'} ({article.likes || 0})
          </button>
        </div>
...
function ArticlesPage({ onOpenAuthModal }) {
    ...
    const loadLikes = async () => {
      ...
      try {
        ...
        const ids = Array.isArray(data)
          ? data.map((item) => String(item.articleId))
          : [];
        console.log('Отримані лайки:', data, 'Збережені ID:', ids);
        setLikedArticleIds(ids);
      } catch (error) {
        setLikedArticleIds([]);
      }
    };
  ...
  const handleLikeSuccess = (articleId) => {
    const normalizedArticleId = String(articleId);

    setLikedArticleIds((prev) => {
      if (prev.includes(normalizedArticleId)) {
        return prev;
      }
      return [...prev, normalizedArticleId];
    });

    setArticles((prev) =>
      prev.map((item) =>
        String(item.id) === normalizedArticleId
        ...
        <div className="articles-grid">
          {articles.map((article) => (
            <ArticleCard
              ...
              isLiked={likedArticleIds.includes(String(article.id))}
